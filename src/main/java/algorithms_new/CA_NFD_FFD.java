...
/**
 * 先高后低，高的采用NFD，低关键层次任务采用FFD
 * **/
public class CA_NFD_FFD {
    public static int CA_NFD_FFD_Partitioning(List<Task> Task_HI, List<Task> Task_LO, List<Task>[] M_List){
            ...
            if (position_HI < count_HI) {
                ...
                while (count_M < M) {
                    ...
                    if (UHI[curr_processor] >= uiHI && ULO[curr_processor] >= uiLO) {
                        M_List[curr_processor].add(Task_HI.get(position_HI));  //把该任务放入处理器
                        if (MC_QPA.Dbf_Schedule(M_List[curr_processor]) == 1) {  //判断当前处理器是否满足可行性条件)
                            ...
                            M_List[curr_processor].remove(Task_HI.get(position_HI));    //从处理器中移除该任务
                        }
