...
public class CU_Averge_WFD {
    public static int CU_Averge_WFD_Partatition(List<Task> taskList, List<Task>[] M_List){
            ...
            for (int i = 1; i <= M; i++) {//遍历处理器
                if (UHI[i] >= uiHI && ULO[i] >= uiLO) {//判断当前任务是否可放入该处理器
                    ...
                    if (MC_QPA.Dbf_Schedule(M_List[i]) == 1) {  //判断当前处理器是否满足可行性条件
                        ...
                        if (Li == 1) {
                            if (max_residual < ((ULO[i] - uiLO)*0.73+(UHI[i] - uiHI))/1.73) {//记录当前处理器利用率的剩余空间和最大剩余空间的处理器位置
                                max_residual = ((ULO[i] - uiLO)*0.73+(UHI[i] - uiHI))/1.73;
                                max_M = i;
                            }
                        } else {
                            if (max_residual < ((ULO[i] - uiLO)+0.73*(UHI[i] - uiHI))/1.73) {//记录当前处理器利用率的剩余空间和最大剩余空间的处理器位置
